...
# Cursor lists tools before Godot may be open; without this, HTTP errors yield "no tools" in the client.
GDAI_OFFLINE_TOOL_NAME: str = "gdai_bridge_status"


def _log_debug(msg: str) -> None:
    # MCP stdio: stdout is reserved for JSON-RPC only. Logging to stdout breaks Cursor (invalid JSON on stdout).
    print(msg, file=sys.stderr, flush=True)


def _offline_tool() -> mt.Tool:
    return mt.Tool(
        name=GDAI_OFFLINE_TOOL_NAME,
        description=(
            "GDAI Godot HTTP bridge is offline. Open Godot 4.x with this project, enable "
            "Project Settings -> Plugins -> GDAI MCP, keep the editor running, then use "
            "Cursor MCP: Restart Servers. Full tool list loads when localhost:%d responds."
...
class GDAIMCPServer(Server):
    ...
    async def _handle_initialized_notification(self, notify: mt.InitializedNotification, session: ServerSession):
        ...
        try:
            ...
            _log_debug("[GDAI MCP] client_initialized skipped (Godot plugin HTTP not up): %s" % ex)
...
async def list_tools() -> list[mt.Tool]:
    try:
        ...
        _log_debug("[GDAI MCP] list_tools: bridge offline, exposing stub tool only: %s" % ex)
        return [_offline_tool()]
...
async def call_tool(name: str, arguments: dict) -> list:
    if name == GDAI_OFFLINE_TOOL_NAME:
        return [mt.TextContent(
            type="text",
            text=(
                "GDAI MCP: the Godot editor plugin is not serving HTTP on %s. "
                "Start Godot, open this project, enable the GDAI MCP plugin, then restart MCP in Cursor."
                % GDAI_HTTP_SERVER_BASE_URL
            ),
        )]
...
async def list_prompts() -> list[mt.Prompt]:
    try:
        ...
        _log_debug("[GDAI MCP] list_prompts: offline - returning empty: %s" % ex)
        return []

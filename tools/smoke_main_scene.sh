...
if [[ -z "$GODOT_BIN" || ! -x "$GODOT_BIN" ]]; then
	if [[ -x "$REPO/Godot_v4.6.1-stable_linux.x86_64" ]]; then
...
if [[ -z "$GODOT_BIN" || ! -x "$GODOT_BIN" ]]; then
	echo "smoke_main_scene.sh: set GODOT to your Godot 4.6+ editor binary, or place Godot_v4.6.1-stable_linux.x86_64 in the repo root." >&2
	exit 1
fi

...
The **exact** number of top-level `signal` declarations in `autoloads/signal_bus.gd` is repeated in prose so standing orders, the master doc, indexes, and this skill stay aligned. **Baseline: 77 (verified 2026-04-20).** Prefer re-counting `^signal ` in that file over guessing.

...
- EVERY new autoload must be registered in `project.godot` in the correct load order:
  ...
  3. AuraManager (no dependencies; tower aura registry)
  4. EconomyManager (depends on SignalBus)
  5. CampaignManager (depends on SignalBus — must load BEFORE GameManager)
  6. RelationshipManager (depends on SignalBus, after CampaignManager)
  7. SettingsManager (depends on SignalBus, after RelationshipManager)
  8. GameManager (depends on SignalBus, EconomyManager, CampaignManager)
  9. BuildPhaseManager (lightweight hex build-phase guard)
  10. AllyManager (depends on SignalBus; summoner-tower AllyBase squads)
  11. CombatStatsTracker (depends on SignalBus, EconomyManager; reads GameManager mission id via signals / optional nodes)
  12. SaveManager (depends on CampaignManager, GameManager, EnchantmentManager)
  13. DialogueManager (depends on SignalBus, GameManager, ResearchManager)
  14. AutoTestDriver (depends on GameManager)
  15. EnchantmentManager (depends on SignalBus)

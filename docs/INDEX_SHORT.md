...
Types	res://scripts/types.gd	All enums and shared constants. Prompt 11: `AllyClass` (MELEE/RANGED/SUPPORT); `TargetPriority` shared with allies (MVP: CLOSEST). Prompt 14: `HubRole` marks between-mission hub character categories. Prompt 32: `BuildingBaseMesh`, `BuildingTopMesh` (modular kit). Prompt 33: `TerrainType`, `TerrainEffect`. Not an autoload; referenced as Types.XXX.
...
BuildingBase	res://scenes/buildings/building_base.gd	res://scenes/buildings/building_base.tscn	Base class for all 8 building types. Auto-targets and fires. Prompt 32: non-default `BuildingData` kit enums → `BuildingKitAssembly` child (`ArtPlaceholderHelper.get_building_kit_mesh`).

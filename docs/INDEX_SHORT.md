...
Compact repository reference. One-liner per file. **Master doc:** `docs/FOUL_WARD_MASTER_DOC.md` — living comprehensive reference for developers and LLM agents (Prompt 53: expanded with full public APIs, lifecycle flows, "how to add X" templates, enum-to-integer tables, and anti-patterns; §1.1 Cursor/MCP toolchain; §23 SimBot headless automation vs GdUnit). **Doc layout:** `docs/README.md`. **Read-only audit aggregate (three-part verification):** `docs/SUMMARY_VERIFICATION.md`. **Archived snapshot (Prompt 26+):** `docs/archived/OPUS_ALL_ACTIONS.md` — historical merge of backlog, AGENTS, PROMPT_26 log, indexes (prefer `FOUL_WARD_MASTER_DOC.md`, repo-root `AGENTS.md`, and `IMPROVEMENTS_TO_BE_DONE.md`). Updated: 2026-03-31 (**Prompt 54:** agent skills index — `AGENTS.md`, `.cursor/skills/*/SKILL.md` — `docs/PROMPT_54_IMPLEMENTATION.md`). (**Prompt 51:** SimBot balance sweep — `SimBot.run_balance_sweep` + `scripts/simbot/simbot_loadouts.gd`, `CombatStatsTracker.begin_run`/`end_run` + CSV `run_label`/`display_name`/`role_tags`, `tools/simbot_balance_report.py`, `tools/apply_balance_status.gd` (`BalanceStatusApplier`), `AutoTestDriver` `--simbot_balance_sweep`, `tests/unit/test_simbot_balance_integration.gd` — `docs/archived/PROMPT_51_IMPLEMENTATION.md`). (**Prompt 10 wave:** `WavePatternData` + `WaveComposer` + `default_campaign_pattern.tres`, staggered composed spawns, `clear_all_enemies` cancels queues, `tests/unit/test_wave_composer.gd` — `docs/archived/PROMPT_10_IMPLEMENTATION.md` §Wave composition). (**Prompt 11:** HUD build menu — full `HexGrid.building_data_registry` roster (`BuildMenuButton`), in-mission `ResearchPanel` + `SignalBus.research_points_changed`/`research_node_unlocked`, `ResearchManager` `can_unlock`/`show_research_panel_for`, `HexGrid` `hex_grid` group, `BuildPhaseManager` `build_phase_started`/`combat_phase_started` + `GameManager` wiring, `tests/unit/test_research_and_build_menu.gd` — `docs/archived/PROMPT_11_IMPLEMENTATION.md`). (**Prompt 09:** enemy `special_tags` runtime — `ShieldComponent`, `AuraManager` enemy `aura_buff`/`aura_heal` registry + stale cleanup, charge/enrage/dash, regen + aura heal tick, saboteur `set_disabled`, anti-air ranged vs flying allies, Brood Carrier `spawn_enemy_at_position`/`get_enemy_data_by_type`, `SignalBus` `enemy_spawned(enemy_type,Vector2)`/`enemy_enraged`, `tests/unit/test_enemy_specials.gd` — `docs/archived/PROMPT_09_IMPLEMENTATION.md`). (**Prompt 07:** `AllyManager` summoner squads + `SignalBus` `ally_died`/`ally_squad_wiped` + extended `ally_spawned`; `BuildingBase` respawn; `HexGrid` soft blockers; `CombatStatsTracker` `ally_deaths`; `tests/unit/test_summoner_runtime.gd` — `docs/archived/PROMPT_07_IMPLEMENTATION.md`). (**Prompt 08:** `AuraManager` autoload + `BuildingData` aura/healer runtime — `damage_pct` / `enemy_speed_pct`, healer `Timer` ticks, `HexGrid` adds `buildings` group before `initialize_with_economy`, `tests/unit/test_aura_healer_runtime.gd` — `docs/archived/PROMPT_08_IMPLEMENTATION.md`). (**Prompt 50:** TD content pass — 36 `BuildingData` + 30 `EnemyData` `.tres`, enums + schema + research + allies + `test_content_invariants.gd` — `docs/archived/PROMPT_50_IMPLEMENTATION.md`). (**Prompt 48:** `CombatStatsTracker` `begin_mission`/`register_building`/`flush_to_disk` + wave/building CSV; `SignalBus` `building_dealt_damage`/`florence_damaged`/`enemy_spawned`; `BuildingBase`/`EnemyBase` stat+aura+status foundation; `ProjectileBase` placed id; `HexGrid` `_register_combat_stats_building`; `SimBot` begin/flush/`debug_batch` — `docs/archived/PROMPT_48_IMPLEMENTATION.md`). (**Prompt 49:** `CombatStatsTracker` String `placed_instance_id` rows + CSV flush; `SignalBus` `building_dealt_damage`/`florence_damaged`/`enemy_spawned`; `BuildingBase` stat layer + summon despawn; `EnemyBase` `receive_damage` + `TRUE` damage; `BuildPhaseManager` autoload + hex build-phase guards + ring rotation; `WaveManager` `enemy_spawned`; `SimBot` `run_batch` `debug_batch`; `anti_air_bolt.tres` `projectile_scene`; tests — `docs/archived/PROMPT_49_IMPLEMENTATION.md`). (**Prompt 47:** economy foundation — `EconomyManager` `_built_counts`/`register_purchase` receipt/`can_afford_building` (no wallet args)/`get_refund` Dictionary/`reset_for_mission`/`passive_gold_per_wave`; `BuildingData.building_id` + `upgrade_next_*`; `BuildingBase.initialize_with_economy`/`get_upgrade_cost` Dictionary; `DayConfig.mission_economy` → `GameManager` — `docs/archived/PROMPT_47_IMPLEMENTATION.md`). (**Prompt 46:** Revert testing-only faction/wave/autotest tweaks; missions **5** waves (`WAVES_PER_MISSION`, `max_waves`, campaigns); bats back in rosters — `docs/archived/PROMPT_46_IMPLEMENTATION.md`). (**Prompt 44:** `EnemyData.matches_tower_air_ground_filter`, `BuildingBase._find_target` uses `get_target_flag_bits` (anti-air vs `body_type`/`is_flying`) — `docs/archived/PROMPT_44_IMPLEMENTATION.md`). (**Prompt 43:** `MissionSpawnRouting` `build_spawn_queue` → `Array[Dictionary]`, `resolve_path_for_spawn` → `RoutePathData`, `validate_*` → `Array[String]`; `WaveManager` queue + `spawn_enemy_on_path` typing — `docs/archived/PROMPT_43_IMPLEMENTATION.md`). (**Prompt 42:** TD core `Resource` definitions (`AllyCombatRole`, `SummonLifetimeType`, `AuraModifierOp`, ring `BuildingSizeClass`, SIEGE/ETHEREAL); `BuildingData` `PackedScene` projectile + enums; `AllyData` `id`/`damage`/`target_flags`; `EnemyData` `scene_path`; `RoutePathData` `NodePath`; `MissionEconomyData` duplicate k `-1`; `BuildingBase`/`EconomyManager`/`CampaignManager`/`WaveManager` wiring; `BuildingBase` projectile `can_instantiate()` — `docs/archived/PROMPT_42_IMPLEMENTATION.md`). (**Prompt 41:** `EconomyManager` linear duplicate scaling per `BuildingData.id` (fallback `building_type:%d`), `get_duplicate_count`/`get_cost_multiplier`/`can_afford_building`, mission duplicate reset; `BuildingBase` upgrade chain `can_upgrade`/`apply_upgrade`, placement `slot_id`/`ring_index`/`placed_instance_id`, guarded projectile scene load; `HexGrid` ring + chain upgrade — `docs/archived/PROMPT_41_IMPLEMENTATION.md`). (**Prompt 40:** `MissionSpawnRouting` typed routing lookup, seeded `spawn_offset_variance_sec`, stricter `validate_wave` (`enemy_data`/`enemy_id`, variance); `WaveManager.resolve_path_for_spawn` casts — `docs/archived/PROMPT_40_IMPLEMENTATION.md`). (**Prompt 39:** TD data resource foundation polish — `BuildingData.get_range`, `AllyData.identity`/`max_lifetime_sec`/`get_identity`, `EnemyData.get_identity`, `SpawnEntryData.enemy_id`, `MissionRoutingData` path→lane validation, `MissionEconomyData`/`path_data` docs; `MissionDataValidation` spawn rows; `tests/unit/test_td_resource_helpers.gd` — `docs/archived/PROMPT_39_IMPLEMENTATION.md`). (**Prompt 38:** `EconomyManager` wave_clear rewards (`get_wave_reward_*`, `grant_wave_clear_reward`, `wave_cleared` autogrant when mission economy set); passive tick unchanged — `docs/archived/PROMPT_38_IMPLEMENTATION.md`). (**Prompt 37:** `EconomyManager` mission economy (`apply_mission_economy`, passive tick, duplicate scaling, `get_refund`), HexGrid `BuildingBase` `paid_*`/`total_invested_*` placement/sell/upgrade, `tests/unit/test_economy_mission_integration.gd` — `docs/archived/PROMPT_37_IMPLEMENTATION.md`). (**Prompt 36:** `WaveManager` mission `MissionData` spawn queue (`DayConfig.mission_data`), `MissionSpawnRouting` + `RoutePathData` bitmask paths, `EnemyBase.assigned_lane_id`/`assigned_path_id`, `tests/unit/test_mission_spawn_routing.gd` — `docs/archived/PROMPT_36_IMPLEMENTATION.md`). (**Prompt 35:** TD data resources — extended `BuildingData`/`AllyData`/`EnemyData`, new `SpawnEntryData`/`WaveData`/`MissionWavesData`/`LaneData`/`PathData`/`MissionRoutingData`/`MissionEconomyData`/`MissionDataValidation`, `Types` TD enums, `mission_spawn_routing` aligned with `spawn_entries`; `docs/archived/PROMPT_35_IMPLEMENTATION.md`). (**Prompt 34:** `CombatStatsTracker` autoload — `user://simbot/runs/{mission_id}_{timestamp}/` wave/building CSVs; `ProjectileBase` attribution; `EnemyBase` emits `enemy_reached_tower`; `docs/archived/PROMPT_34_IMPLEMENTATION.md`). (**Prompt 33:** terrain system — `Types.TerrainType`/`TerrainEffect`, `TerrainZone`, `NavMeshManager` autoload, `CampaignManager._load_terrain`, `terrain_grassland.tscn`/`terrain_swamp.tscn`, `main.tscn` `TerrainContainer`, `tests/unit/test_terrain.gd`, `FUTURE_3D_MODELS_PLAN.md` §5 — `docs/archived/PROMPT_33_IMPLEMENTATION.md`). (**Prompt 32:** modular building kit — `Types.BuildingBaseMesh`/`BuildingTopMesh`, `BuildingData` kit exports, `ArtPlaceholderHelper.get_building_kit_mesh`, `BuildingBase` `BuildingKitAssembly`, `tests/unit/test_building_kit.gd`, `FUTURE_3D_MODELS_PLAN.md` §4 — `docs/archived/PROMPT_32_IMPLEMENTATION.md`). (**Prompt 31:** rigged GLB mount under `EnemyVisual` / `BossVisual` / `ArnulfVisual`; `RiggedVisualWiring` + idle/walk `AnimationPlayer`; `test_art_placeholders` deep mesh find — `docs/archived/PROMPT_31_IMPLEMENTATION.md`). **Prompt 29:** art pipeline audit verification — `FUTURE_3D_MODELS_PLAN.md` Appendix A (scene grep + helper API); Godot MCP `reload_project` + editor error scan; `# TODO(ART)` on `boss_base.tscn` — `docs/archived/PROMPT_29_IMPLEMENTATION.md`). **Prompt 28:** hex pick fallback + slot collider height; Arnulf leash / immobile ignore; build menu min size; `GameManager` save lambda; `DialogueManager` tracking + `WaveManager` BUILD_MODE countdown pause; `test_relationship_manager_tiers.gd` / `test_save_manager_slots.gd`; consumables + ally combat test fixes — `docs/archived/PROMPT_28_IMPLEMENTATION.md`). **Prompt 26:** Full project audit — 55 unindexed files added, AGENTS.md standing orders, IMPROVEMENTS_TO_BE_DONE.md backlog, test classification — `docs/archived/PROMPT_26_IMPLEMENTATION.md`. **Prompt 24:** programmatic PNG icons (`tools/generate_placeholder_icons.gd`, `addons/fw_placeholder_icons`), `ArtPlaceholderHelper` icon textures, `SettingsManager` + `scenes/ui/settings_screen`, UI wiring — `docs/archived/PROMPT_24_IMPLEMENTATION.md`. **Prompt 23:** Endless Run — `Types.GameState.ENDLESS`, `CampaignManager.is_endless_mode` / `start_endless_run`, synthetic day scaling, main menu — `docs/archived/PROMPT_23_IMPLEMENTATION.md`. **Prompt 22:** relationship tiers + affinity autoload, dialogue `relationship_tier` conditions — `docs/archived/PROMPT_22_IMPLEMENTATION.md`. Prompt 19: Blender batch GLBs under `res://art/generated/**`, `generation_log.json`, `FUTURE_3D_MODELS_PLAN.md`, `# TODO(ART)` in combat/hub scenes; see `docs/archived/PROMPT_19_IMPLEMENTATION.md`. **Prompt 18:** local RAG + MCP pipeline under `~/LLM` — `docs/archived/PROMPT_18_IMPLEMENTATION.md`. **Audit 6:** `AUDIT_IMPLEMENTATION_AUDIT_6.md` (multi-spell, structural weapon upgrades, barracks/shield specials, territory aggregates). **Prompt 20:** `docs/obsolete/` archive + INDEX autoload alignment; `docs/archived/PROMPT_20_IMPLEMENTATION.md`.
Source of truth: REPO_DUMP_AFTER_MVP.md; **re-run** `./tools/run_gdunit.sh` after Prompt 12/13 (use `./tools/run_gdunit_quick.sh` for iteration). **Handoff:** `docs/archived/PROBLEM_REPORT.md` lists files and log snippets for GdUnit / `mission_won` / `push_warning` work.
...
tools/complete_comfyui_assets.sh	tools/complete_comfyui_assets.sh	Prompt 78: after ComfyUI at `~/ComfyUI`, downloads gated FLUX.1-dev (`HF_TOKEN`) + CivitAI LoRAs (optional `CIVITAI_TOKEN`); see `docs/gen3d_workplan.md`.
...
LATEST CHANGES (2026-03-25)
    ...
    - Prompt 13 hub dialogue (`docs/archived/PROMPT_13_IMPLEMENTATION.md`): `DialogueManager` autoload; `DialogueEntry` / `DialogueCondition`; `res://resources/dialogue/**` pools; `dialogue_ui.tscn`; `UIManager.show_dialogue_for_character` + queue; `BetweenMissionScreen` hub lines for Sybil + Arnulf; `test_dialogue_manager.gd` + `run_gdunit_quick.sh` allowlist.

    - Prompt 12 mercenary roster + offers (`docs/archived/PROMPT_12_IMPLEMENTATION.md`): `MercenaryOfferData`, `MercenaryCatalog`, `MiniBossData`, `res://resources/mercenary_catalog.tres` + offers; `CampaignManager` purchase/preview/defection/auto-select; `SignalBus` mercenary + roster signals; `BetweenMissionScreen` Mercenaries tab; `SimBot` strategy + `decide_mercenaries`; GdUnit suites in `run_gdunit_quick.sh` allowlist; `GameManager._transition_to` idempotent for same state.

LATEST CHANGES (2026-03-24)

    - Prompt 10 fixes (`docs/archived/PROMPT_10_FIXES.md`): WaveManager `get_node_or_null` for EnemyContainer/SpawnPoints; `test_wave_manager` / `test_boss_waves` add SpawnPoints to tree before Marker3D `global_position`; `WeaponLevelData` `.tres` `script_class` header; `test_campaign_manager` GdUnit `assert_that().is_not_null()`; `GameManager` `push_warning` + `mission_won` hub (`project.godot` CampaignManager before GameManager); `docs/archived/PROBLEM_REPORT.md` for errors/snippets.
- Prompt 10 mini-boss + campaign boss (`docs/archived/PROMPT_10_IMPLEMENTATION.md`):
...
- Prompt 9 factions + weighted waves (`docs/archived/PROMPT_9_IMPLEMENTATION.md`):
...
- Implementation notes recorded in `docs/archived/PROMPT_1_IMPLEMENTATION.md`.
...
- Implementation notes recorded in `docs/archived/PROMPT_2_IMPLEMENTATION.md`.
- Phase 3 weapon-upgrade system added:
  ...
  - `docs/archived/PROMPT_3_IMPLEMENTATION.md` records implementation details

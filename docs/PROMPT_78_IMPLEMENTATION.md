...
- Updated parent `gen3d_workplan.md` Step 1: replaced deprecated `huggingface-cli download` with `hf download` + `HF_TOKEN` / license note.
...
3. Run `./tools/complete_comfyui_assets.sh` from repo root (or run `hf download` / manual LoRA copies per `gen3d_workplan.md`).  

...
**Master doc:** `docs/FOUL_WARD_MASTER_DOC.md`. **Audit aggregate:** `docs/SUMMARY_VERIFICATION.md`. **Session logs:** 10-file rolling window in `docs/` (**`PROMPT_80`…`PROMPT_89`** as of 2026-04-20); full history in `docs/archived/prompts/` (**90** `PROMPT_*_IMPLEMENTATION.md` files via `find docs docs/archived/prompts -maxdepth 1 -name 'PROMPT_*_IMPLEMENTATION.md'`). **Source of truth for behaviour:** the repository (code + tests), not deleted REPO_DUMP exports.

Use `INDEX_SHORT.md` for fast orientation; this file for exact method signatures, signals, and dependencies.
...
Consumes: all_waves_cleared, tower_destroyed, boss_killed; subscribes to mission_won (hub transition). Autoload order: CampaignManager before GameManager so day increments before hub transition.
BuildPhaseManager
...
Internal: `_evaluate_conditions`, `_resolve_state_value`, `_compare`, `_sybil_research_unlocked_any`, `_arnulf_research_unlocked_any`, `_get_research_manager()` — see `docs/archived/prompts/PROMPT_13_IMPLEMENTATION.md` for condition keys. Combat keys: `first_blood`, `wave_number_gte`, `kills_this_mission_gte`, `boss_active`, `florence_damaged`.
...
Art pipeline reference (Session 07 Perplexity reports removed 2026-04-20)

- Use `RiggedVisualWiring` + `tools/validate_art_assets.gd` for clip/path expectations; session write-ups were one-off exports superseded by code + `docs/FUTURE_3D_MODELS_PLAN.md`.
...
Path: docs/FUTURE_3D_MODELS_PLAN.md  
...
- **Notes**: `docs/archived/prompts/PROMPT_9_IMPLEMENTATION.md`.
...
- See `docs/archived/prompts/PROMPT_1_IMPLEMENTATION.md` for implementation-specific details.
...
- See `docs/archived/prompts/PROMPT_2_IMPLEMENTATION.md` for full Phase 2 implementation and test notes.
- Added deterministic weapon-upgrade station Phase 3:
  ...
  - See `docs/archived/prompts/PROMPT_3_IMPLEMENTATION.md` for full implementation notes.
...
- See `docs/archived/prompts/PROMPT_8_IMPLEMENTATION.md`.

## 2026-03-24 Prompt 10 delta (mini-boss + campaign boss)

- **Implementation notes**: `docs/archived/prompts/PROMPT_10_IMPLEMENTATION.md`.
...
- Historical note: `PROMPT_10_FIXES.md` was a one-off handoff file (removed 2026-04-20); see **`docs/archived/prompts/PROMPT_10_IMPLEMENTATION.md`** and git history if needed.
...
- **Implementation notes**: `docs/archived/prompts/PROMPT_27_IMPLEMENTATION.md`.
...
- **New test runners**:
  ...
  - `tools/run_gdunit_parallel.sh` — 8-parallel-process runner for all test files (~88), ~2m45s wall-clock.
- **Deleted (historical)**: root-level audit task files (superseded by `docs/SUMMARY_VERIFICATION.md` and ongoing prompt logs; large repo dumps and scratch audits removed 2026-04-20 — use `git log` if needed).
...
- **`INDEX_SHORT.md`** — added rows for `stage1_image.py`, `stage4_anim.py`, `stage5_drop.py`, `secrets_loader.py`, `scripts/trellis2_input_ab_variant.py`, `art/gen3d_candidates/`, `art/gen3d_previews/`; rolling-window blurb → **`PROMPT_80`…`PROMPT_89`**.
- **`HOW_IT_WORKS.md`**, **`INTERVIEW_CHEATSHEET.md`**, **`AGENTS.md`**, **`docs/README.md`**, **`docs/ARCHITECTURE.md`**, **`docs/CONVENTIONS.md`**, **`docs/archived/prompts/README.md`**, **`SUMMARY_VERIFICATION.md`** (HexGrid slot count) — metrics and dates aligned.
- Rolling window: **`docs/PROMPT_79_IMPLEMENTATION.md`** → **`docs/archived/prompts/`**; new **`docs/PROMPT_89_IMPLEMENTATION.md`**.

## 2026-04-20 Prompt 86 delta (gen3d texture fix + 5-variant selection)

### tools/gen3d/pipeline/stage2_mesh.py
- `decimate_glb(input_path, out_path, target_faces=10000)` — rewrote to preserve UV/PBR textures: Open3D QEM decimation for correct face-count targeting, then scipy `cKDTree` nearest-vertex re-projection of UV (for `TextureVisuals`) or vertex colors (for `ColorVisuals`) from original high-res mesh. Exports via `trimesh.Scene` so embedded textures round-trip.
- `_load_combined_mesh(input_path)` — helper: load GLB as `trimesh.Scene`, merge geometries, preserve visual data.
...
- `run_pipeline` — rewired Stage 2: calls `generate_mesh_variants` + `select_candidate`; writes `meta.json` sidecar. Honors `SKIP_STAGE1=1`, `SKIP_STAGE2=1`, `SELECTED_GLB=<path>` env vars for `promote_candidate.py` re-runs.

### tools/gen3d/pipeline/stage3_rig.py
- `_load_mixamo_credentials()` — reads `MIXAMO_EMAIL`/`MIXAMO_PASSWORD` from env (already populated by `secrets_loader`) or directly from `~/.foulward_secrets`. Raises `RuntimeError` if neither is set. Never logs the password.
- `rig_model` — updated to call `_load_mixamo_credentials()` when caller-supplied credentials are empty strings. Default arg values changed to `""`.

### tools/gen3d/promote_candidate.py *(new)*
- CLI: `python3 promote_candidate.py <slug> <variant_N>`. Copies `candidate_N_decimated.glb` → `selected.glb`, updates `meta.json`, re-invokes `foulward_gen.py` with `SKIP_STAGE1=1 SKIP_STAGE2=1 SELECTED_GLB=<path>`.

### tools/gen3d/generate_all.sh
- Added `AUTO_SELECT_CANDIDATE=1` and `N_MESH_VARIANTS=5` default exports so batch runs auto-select candidate 1 without prompting.

### tools/gen3d/requirements.txt
- Added `fast_simplification>=0.1.7` (installed 0.1.13 in `trellis2`; not used in the final decimation path due to topology limits of TRELLIS meshes, but available as a dep for future use).
